...
if nf == 2:
    ...
    circleA = plt.Circle((cA_train[0], cA_train[1]), best_params['epsilon'], color='black', fill=False)
    circleB = plt.Circle((cB_train[0], cB_train[1]), best_params['epsilon'], color='black', fill=False)
    axes.add_artist(circleA)
    axes.add_artist(circleB)
    axes.set_aspect(1)
    plt.axis('equal')
    axes.legend(handles=[a_scatter,b_scatter])
    #plt.title("Spherical Classification - n_samples = "+str(ns)+", n_features = "+str(nf))
    plt.savefig('experiments/dense_class_sel_fig_'+str(ns)+'_'+str(nf)+'.pdf')